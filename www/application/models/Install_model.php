...
class Install_model extends CI_Model
{
  ...
  public function __construct()
  {
    ...
    //テーブル名配列のセット
    $this->tables = array('admin', 'attribute', 'attribute_tree', 'attribute_relationship', 'contact', 'contact_meta', 'comment', 'comment_tree', 'comment_meta', 'content', 'content_tree', 'content_meta', 'directory','directory_tree', 'option', 'resource', 'task', 'user', 'user_relationship');
  }
  ...
  public function install($truncate = FALSE)
  {
    ...
    ( 
      ...
      `type_index` INT(2) NOT NULL COMMENT '権限種類' , 
    ...
    $results['directory_tree'] = $this->db->query("
    CREATE TABLE IF NOT EXISTS 
     `{$this->db->dbprefix}directory_tree` 
     ( `ancestor` INT(9) NOT NULL COMMENT '祖先' , 
     `descendant` INT(9) NOT NULL COMMENT '子孫' , 
     PRIMARY KEY (`ancestor`, `descendant`), KEY `descendant` (`descendant`), 
     CONSTRAINT `directory_ancestor_tree` FOREIGN KEY (`ancestor`) REFERENCES `{$this->db->dbprefix}directory` (`id`), 
     CONSTRAINT `directory_descendant_tree` FOREIGN KEY (`descendant`) REFERENCES `{$this->db->dbprefix}directory` (`id`)
   ) ENGINE = InnoDB CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT = 'directoryテーブルの階層構造対する閉包テーブル';
   ");
    ...
    ( 
      ...
      `type_index` INT(2) NOT NULL DEFAULT 0 COMMENT '種類' , 
      `order_number` INT(9) NOT NULL COMMENT '順序' , 
    ...
    $results['content_tree'] = $this->db->query("
    CREATE TABLE IF NOT EXISTS 
     `{$this->db->dbprefix}content_tree` 
     ( `ancestor` INT(9) NOT NULL COMMENT '祖先' , 
     `descendant` INT(9) NOT NULL COMMENT '子孫' , 
     PRIMARY KEY (`ancestor`, `descendant`), KEY `descendant` (`descendant`), 
     CONSTRAINT `content_ancestor_tree` FOREIGN KEY (`ancestor`) REFERENCES `{$this->db->dbprefix}content` (`id`), 
     CONSTRAINT `content_descendant_tree` FOREIGN KEY (`descendant`) REFERENCES `{$this->db->dbprefix}content` (`id`)
   ) ENGINE = InnoDB CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT = 'contentテーブルの階層構造対する閉包テーブル';
   ");
    ...
    $results['attribute_tree'] = $this->db->query("
    CREATE TABLE IF NOT EXISTS 
     `{$this->db->dbprefix}attribute_tree` 
     ( `ancestor` INT(9) NOT NULL COMMENT '祖先' , 
     `descendant` INT(9) NOT NULL COMMENT '子孫' , 
     PRIMARY KEY (`ancestor`, `descendant`), KEY `descendant` (`descendant`), 
      CONSTRAINT `attribute_ancestor_tree` FOREIGN KEY (`ancestor`) REFERENCES `{$this->db->dbprefix}attribute` (`id`), 
      CONSTRAINT `attribute_descendant_tree` FOREIGN KEY (`descendant`) REFERENCES `{$this->db->dbprefix}attribute` (`id`)
   ) ENGINE = InnoDB CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT = 'attributeテーブルの階層構造対する閉包テーブル';
   ");
   ...
   ");
    ...
    $results['comment_tree'] = $this->db->query("
    CREATE TABLE IF NOT EXISTS 
     `{$this->db->dbprefix}comment_tree` 
     ( `ancestor` INT(9) NOT NULL COMMENT '祖先' , 
     `descendant` INT(9) NOT NULL COMMENT '子孫' , 
     PRIMARY KEY (`ancestor`, `descendant`), KEY `descendant` (`descendant`), 
      CONSTRAINT `comment_ancestor_tree` FOREIGN KEY (`ancestor`) REFERENCES `{$this->db->dbprefix}comment` (`id`), 
      CONSTRAINT `comment_descendant_tree` FOREIGN KEY (`descendant`) REFERENCES `{$this->db->dbprefix}comment` (`id`)
   ) ENGINE = InnoDB CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci COMMENT = 'commentテーブルの階層構造対する閉包テーブル';
   ");
